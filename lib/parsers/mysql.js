...
var MysqlParser = function () {
	function MysqlParser(data) {
		_classCallCheck(this, MysqlParser);

		// dbUser, dbPassword, dbName, dbHost = 'localhost', excludeTables) {
		console.log(data);
	...
	_createClass(MysqlParser, [{
		...
		value: async function generateFiles() {
			try {
				var initialArgs = 'sequelize initial --name ' + this.appName + ' --dbHost ' + this.dbHost + ' --dbName ' + this.dbName + ' --dbUser ' + this.dbUser + ' --dbPassword ' + this.dbPassword;
				(0, _writter2.default)(initialArgs);
